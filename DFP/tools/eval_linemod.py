...
def main():
    ...
    parser.add_argument(
        "--refine_model", type=str, default="", help="resume PoseRefineNet model"
    )
    ...
    testdataset = PoseDataset_linemod(
        "eval", num_points, False, opt.dataset_root, 0.0, True
    )
    ...
    for i, data in enumerate(testdataloader, 0):
        ...
        my_t = (
            (points.view(bs * num_points, 1, 3) + pred_t)[which_max[0]]
            .view(-1)
            .data.numpy()
        )
        ...
        for ite in range(0, iteration):
            ...
            my_r = my_r_final
            my_t = my_t_final
            print(f"my_pred is {my_pred}")

        model_points = model_points[0].detach().numpy()
        ...
        if idx[0].item() in sym_list:
            pred = (
                torch.from_numpy(pred.astype(np.float32)).transpose(1, 0).contiguous()
            )
            target = (
                torch.from_numpy(target.astype(np.float32)).transpose(1, 0).contiguous()
            )
            inds = knn(target.unsqueeze(0), pred.unsqueeze(0))
            target = torch.index_select(target, 1, inds.view(-1) - 1)
            dis = torch.mean(
                torch.norm((pred.transpose(1, 0) - target.transpose(1, 0)), dim=1),
                dim=0,
            ).item()
        else:
    ...
    for i in range(num_objects):
        print(
            "Object {0} success rate: {1}".format(
                objlist[i], float(success_count[i]) / num_count[i]
            )
        )
        fw.write(
            "Object {0} success rate: {1}\n".format(
                objlist[i], float(success_count[i]) / num_count[i]
            )
        )
