...
refiner.load_state_dict(torch.load(opt.refine_model, map_location=torch.device("cpu")))
refiner.eval()
...
for now in range(0, 2949):
    ...
    depth = np.array(
        Image.open("{0}/{1}-depth.png".format(opt.dataset_root, testlist[now]))
    )
    posecnn_meta = scio.loadmat(
        "{0}/results_PoseCNN_RSS2018/{1}.mat".format(ycb_toolbox_dir, "%06d" % now)
    )
    ...
    scio.savemat(
        "{0}/{1}.mat".format(result_refine_dir, "%04d" % now), {"poses": my_result}
    )
