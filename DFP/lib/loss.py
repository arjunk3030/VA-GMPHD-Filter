...
from DFP.lib.knn.__init__ import KNearestNeighbor
...
def loss_calculation(
    ...
    if not refine:
        if idx[0].item() in sym_list:
            ...
            pred = (
                pred.view(3, bs * num_p, num_point_mesh).permute(1, 2, 0).contiguous()
            )
...
class Loss(_Loss):
    ...
    def forward(
        self, pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine
    ):
