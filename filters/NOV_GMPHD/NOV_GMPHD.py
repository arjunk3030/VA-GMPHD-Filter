...
class NOV_GaussianMixture:
    ...
    def copy(self):
        ...
        for P1 in self.P:
            P.append(P1.copy())
        return NOV_GaussianMixture(w, m, P, cls)
...
def thinning_and_displacement(v: NOV_GaussianMixture, p, F: np.ndarray, Q: np.ndarray):
    """
    ...
    return NOV_GaussianMixture(w, m, P, v.cls)


def just_displacement(v: NOV_GaussianMixture, F: np.ndarray, Q: np.ndarray):
    """
    For the given Gaussian mixture v, perform thinning with probability P and displacement with N(x; F @ x_prev, Q)
    See https://ieeexplore.ieee.org/document/7202905 for details
    """
    ...
    for cov_matrix in v.P:
        P.append(Q + F @ cov_matrix @ F.T)
    return NOV_GaussianMixture(v.w, m, P, v.cls)


def spread_convariance(v: NOV_GaussianMixture, Q: np.ndarray):
    w = []
    m = []
    P = []
...
class NOV_GMPHD:
    ...
    def prediction(self, v: NOV_GaussianMixture) -> NOV_GaussianMixture:
        """
        ...
        return NOV_GaussianMixture(v_s.w, v_s.m, v_s.P, v.cls)

    def correction(
        self, v: NOV_GaussianMixture, p_v, Z: List[np.ndarray], Zcls, distance
    ) -> NOV_GaussianMixture:
        """
    ...
    def pruning(self, v: NOV_GaussianMixture) -> NOV_GaussianMixture:
        """
        See https://ieeexplore.ieee.org/document/7202905 for details
        """
        ...
        v = NOV_GaussianMixture(w, m, P, cls)
    ...
    def state_estimation(self, v: NOV_GaussianMixture) -> List[np.ndarray]:
        X = []
        Xc = []
